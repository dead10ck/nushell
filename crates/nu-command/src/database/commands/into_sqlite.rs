...
impl Table {
    ...
    fn try_init(
        ...
        self.conn.execute(&create_statement, []).map_err(|e| {
            ShellError::GenericError(
                "Failed to create table".into(),
                e.to_string(),
                None,
                None,
                Vec::new(),
...
fn insert_in_transaction(
    ...
    insert_statement.finalize().map_err(|e| {
        ShellError::GenericError(
            "Failed to finalize SQLite prepared statement".into(),
            e.to_string(),
            None,
            None,
            Vec::new(),
    ...
    tx.commit().map_err(|e| {
        ShellError::GenericError(
            "Failed to commit SQLite transaction".into(),
            e.to_string(),
            None,
            None,
            Vec::new(),
...
fn insert_value(
    ...
    match stream_value {
        ...
        Value::Record { val, .. } => {
            let sql_vals = values_to_sql(val.vals)?;
